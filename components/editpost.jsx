...
export default function Editpost ({ countries = [] }) {
  const { postImg, setPostImg, description, inputImg, user, AddPost, postType, country, setcountry, city, Cities, disabled } = UseEditPost()
      ...
      <div className='w-full px-4 flex gap-3'>
        ...
        <div className='w-full'>
          ...
          <div className='flex justify-between items-end'>
            ...
            <button disabled={disabled} onClick={() => AddPost(country, city)} className='px-4 disabled:bg-sky-500 py-2 rounded-l-full rounded-r-full bg-sky-700 text-gray-200'>Editar</button>
          </div>
        </div>
      </div>
    </div>
