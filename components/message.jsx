...
export default function Message ({ currentConversation = null }) {
  ...
  if (currentConversation?.Conversation_ID && loading) return <div className='w-full h-full rounded-md bg-black grid place-items-center'><div className='w-10 h-10'><Loading /></div></div>
  return (
      ...
      <div className='h-[calc(100%-92px)] overflow-y-scroll py-6 px-20'>
        ...
        messages?.map((message) => (
          <div ref={scrollRef} className={`w-full flex gap-1.5 mb-2 ${message?.Sender_ID === userFound?.User_ID ? '' : 'flex-row-reverse'}`} key={message.Message_ID}>
            ...
            <p className={`h-10 rounded-md p-4 flex items-center font-medium text-[#f2f2f2] ${message?.Sender_ID === userFound?.User_ID ? 'bg-sky-700' : 'bg-gray-700'}`}>{message.description}</p>
          </div>
        ))
      }
      </div>
